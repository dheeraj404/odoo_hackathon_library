...
export default function Dashboard() {
  ...
  const handleAddLibrarian = async (values) => {
    try {
      const response = await fetch("http://localhost:5000/api/user/register-new-user", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleSearch = async (searchType, searchValue) => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/book/${endpointMap[searchType]}/${searchValue}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
  ...
  const handleGetAllBooks = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:5000/api/book/get-all-books", {
        headers: {
  ...
  const handleRequestIssue = async (bookId) => {
    try {
      const response = await axios.post(`http://localhost:5000/api/book/request-to-issue-book/${bookId}`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      ...
      if (response.data.success) {
        alert("Book issue requested successfully");
      } else {
        console.log(response.data.message);
      }
    } catch (error) {
    ...
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <main className="flex flex-1 p-6">
        <div className="w-full max-w-6xl mx-auto bg-white shadow-lg rounded-lg p-6 space-y-6">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4">
            Welcome, {userData.role === "admin" ? "Admin" : userData.name}!
          </h2>
          ...
          <div className="my-6">
            ...
            <div className="flex space-x-4 mb-4">
              ...
              />
              <button
                onClick={() => handleSearch("title", document.getElementById("search").value)}
                className="px-4 py-2 bg-blue-500 text-white rounded-lg"
              >
                Search by Title
              </button>
              <button
                onClick={() => handleSearch("author", document.getElementById("search").value)}
                className="px-4 py-2 bg-blue-500 text-white rounded-lg"
              >
                Search by Author
              </button>
              <button
                onClick={() => handleSearch("genre", document.getElementById("search").value)}
                className="px-4 py-2 bg-blue-500 text-white rounded-lg"
              >
                Search by Genre
              </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {searchResults.map((book) => (
                <div key={book._id} className="bg-gray-50 p-4 shadow rounded-lg">
                  <img src={book.small_icon} alt={book.title} className="mb-4 w-full h-auto" />
      ...
      >
        <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4">Add Librarian</h2>
          <Formik
